...
const endpoint = "543ae742255f479dbe630e3c2aef8488";
...
async function updateCart(id, data) {
  const resopnse = await fetch(
    `https://crudcrud.com/api/${endpoint}/mycart/${id}`,
    {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
...
function App() {
  ...
  useEffect(() => {
    if (orderList.length !== 0) updateCart(crudcrudUpdateID.current, orderList);
  }, [orderList]);
